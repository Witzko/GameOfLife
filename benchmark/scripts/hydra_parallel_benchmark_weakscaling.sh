...
#SBATCH --time=10:00
#SBATCH --output=benchmark/unformatted_outputs/parallel_benchmark_weakscaling_alltoall_ugly.out
    ...
    for prob_of_life in "${prob_lives[@]}"; do
        for num_of_repetitions in "${repetitions[@]}"; do
            for num_of_processes in "${processes[@]}"; do
                ...
                mpirun -np $(($num_of_processes*32)) $executable $N $M $prob_of_life $num_of_repetitions true $num_of_processes 32
            done
        done
    done
done